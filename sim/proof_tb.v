...
// Proof testbench:
// Compares the lightweight material evaluator against a simple NN-based
// evaluator on a few handcrafted positions. The goal is to see cases where
// material_score is similar but the NN (deep_score) differs because of
// positional nuance.

module proof_tb;

    // ROMs for weights and biases (loaded from training artifacts)
    ...
    reg clk = 0;
    reg reset = 0;
    always #5 clk = ~clk;  // 100 MHz-ish for simulation timing

    // Simple MAC interface signals
    ...
    // Activation vector for the NN input (flattened 768-bit representation)
    reg [767:0] act_vector;

    // Sum of hidden neuron outputs (simple aggregator for a proof-of-concept)
    reg signed [63:0] deep_score;

    integer n_idx;
    integer i_idx;

    // Run one hidden neuron: stream 768 activations through the MAC
    ...
    // Evaluate a single position: load bitboards, run all neurons
    task eval_position(
    ...
    initial begin
        ...
        // Load trained parameters (paths relative to where simulation is run)
        ...
        // Position 2: passed pawn close to promotion
        ...
        // Position 3: poor knight placement for black
